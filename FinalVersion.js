...
function App() {
  ...
  const handleLangChange = () => {
    if (isListening == false) {
      setLang((prevLang) => (prevLang === "bn-BD" ? "en-US" : "bn-BD"));
    } else {
      alert(" stop Listening please ");
    }
  };

  return (
    <div className="container">
      <h1>{lang === "bn-BD" ? "ভয়েস নোটস" : "Voice Notes"} </h1>
      <button onClick={handleLangChange}>
        {lang === "bn-BD" ? "Switch to English" : "বাংলা চালু করুন"}
      </button>
      <div className="box">
        <label htmlFor="note">
          {" "}
          {lang === "bn-BD" ? "বাংলা নোট:" : "English Note:"}
        </label>
        <textarea
          id="note"
          value={note}
